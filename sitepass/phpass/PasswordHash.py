#!/usr/bin/env python
'''
Direct translation of PHPass from PHP to Python.  Following are the notes from
the original:
...
class PasswordHash(object):
    def __init__(self, iteration_count_log2, portable_hashes):
        '''
        portable_hashes
        '''
    ...
    def get_random_bytes(count):
        ...
        if is_readable('/dev/urandom') and fh:
        ...
        for i in range(16):
            ...
            output.append(pack('H*', md5(self.random_state)))
            output = substr(output, 0, count)
            return ''.join(output)

    def encode64(input, count):
        '''
        The original wasn't written to use base64_encode because of a
        different character set requirement.
        '''
        output = []
        i = 0
        while True:
    ...
    def gensalt_private(input):
        ...
        output += (
            self.itoa64[min(
                self.iteration_count_log2 + (5 if (PHP_VERSION >= '5') else 3),
                30
            )]
        )
    ...
    def CheckPassword(password, stored_hash):
